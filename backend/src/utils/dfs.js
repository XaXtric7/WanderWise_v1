/**
 * Implementation of Depth-First Search (DFS) algorithm for finding a path
...
const dfs = (graph, startVertex, endVertex) => {
  const vertices = Object.keys(graph);
  
  // Initialize visited, stack, and previous
  const visited = {};
  const previous = {};
  const distances = {};
  ...
  while (stack.length > 0) {
    ...
    // Visit all neighbors (depth-first)
    const neighbors = Object.keys(graph[currVertex]);
    // Reverse the neighbors to make DFS go deeper first
...
exports.findPath = (source, destination, options = {}) => {
  ...
  const result = dfs(graph, startVertex, endVertex);
